...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('filtro-data').addEventListener('change', carregarConsultas);
...
async function carregarConsultas() {
  ...
  if (!r.success) { lista.innerHTML = '<p class="vazio">Não foi possível carregar a agenda.</p>'; return; }
...
function renderizarConsultas(consultas) {
  const lista = document.getElementById('lista-consultas');
  if (consultas.length === 0) { lista.innerHTML = '<p class="vazio">Nenhuma consulta encontrada.</p>'; return; }
  lista.innerHTML = '';
  consultas.forEach(c => lista.appendChild(criarCardConsulta(c)));
}
...
function criarCardConsulta(c) {
  const div = document.createElement('div');
  div.className = 'card-consulta status-' + c.status;
  const mostrarNomeProfissional = sessao.perfil === 'secretaria' && !document.getElementById('filtro-profissional').value;

  div.innerHTML = `
    <div class="horario">${c.horaInicio} – ${c.horaFim}</div>
    <div class="detalhes">
      <div class="paciente">${escapeHtml(c.paciente)}</div>
      ${mostrarNomeProfissional ? `<div class="obs">${escapeHtml(nomeProfissional(c.profissionalId))}</div>` : ''}
      ${c.observacoes ? `<div class="obs">${escapeHtml(c.observacoes)}</div>` : ''}
      ${c.valor ? `<div class="obs">R$ ${Number(c.valor).toFixed(2)} · ${c.statusPagamento === 'pago' ? 'Pago' : 'Pendente'}</div>` : ''}
    </div>
    <span class="tag-status">${rotuloStatus(c.status)}</span>
    <div class="acoes">
      <button class="btn-texto btn-editar">Editar</button>
      ${c.status !== 'cancelado' ? '<button class="btn-texto btn-desmarcar" style="color:#A64B3B">Desmarcar</button>' : ''}
    </div>
  `;
  div.querySelector('.btn-editar').addEventListener('click', () => abrirModal(c));
  const btnDesmarcar = div.querySelector('.btn-desmarcar');
  if (btnDesmarcar) btnDesmarcar.addEventListener('click', () => desmarcarConsulta(c.id));
  return div;
}
...
async function buscarPorPaciente() {
  ...
  if (r.success) renderizarConsultas(r.consultas.sort((a, b) => (b.data + b.horaInicio).localeCompare(a.data + a.horaInicio)));
}
...
function abrirModal(consulta) {
  ...
  document.getElementById('consulta-hora-inicio').value = consulta ? consulta.horaInicio : '';
  document.getElementById('consulta-hora-fim').value = consulta ? consulta.horaFim : '';
